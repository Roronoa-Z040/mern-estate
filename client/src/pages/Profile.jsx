    ...
    <div className="p-3 max-w-lg mx-auto">
      ...
      <form onSubmit={handleSubmit}  className="flex flex-col gap-4">
        ...
        <p className="text-sm self-center">
          {fileUploadError ? (
            <span className="text-red-700">  Image upload Error ! (Image must be less than 2 mb)
            </span> 
     ...
     {/*  <p className="text-red-700 mt-5">{error ? error : '' }</p>   */}      
      ...
      <button onClick={handleShowListings} className="text-green-700 w-full uppercase">
        Show listings
      </button> 
      ...
      {userListings &&  
        ...
        {userListings.map((listing ) => (
            ...
            <div key={listing._id} className='border  border-gray-300  rounded-lg p-3 flex justify-between items-center gap-4'>
              ...
              <Link className="text-slate-700 font-semibold hover:underline truncate flex-1" to={`/listing/${listing._id}`}>{listing.title}
                <p className="">
                  {listing.name}
                </p>
              </Link>
